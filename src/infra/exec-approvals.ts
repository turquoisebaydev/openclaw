...
// Filenames only — directory comes from resolveStateDir() to respect OPENCLAW_STATE_DIR / profile.
...
export function resolveExecApprovalsPath(env?: NodeJS.ProcessEnv): string {
  return path.join(resolveStateDir(env), APPROVALS_FILENAME);
}

export function resolveExecApprovalsSocketPath(env?: NodeJS.ProcessEnv): string {
